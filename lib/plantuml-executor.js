...
module.exports.useNailgun = function (callback) {
  ...
  nailgunServer = nailgun.createServer(options, function (port) {
    console.log('Nailgun started', LOCALHOST, port)
    clientOptions = {
      host: LOCALHOST,
      port: port
    }
    ...
    ngClient.exec('ng-cp', [PLANTUML_JAR], clientOptions).on('exit', () => {
      ngClient.exec('ng-cp', [PLANTUML_NAIL_JAR], clientOptions).on('exit', () => {
